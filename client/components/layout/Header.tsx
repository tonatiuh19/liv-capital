...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-stone-warm/30 transition-shadow duration-300 ${scrolled ? "" : "shadow-md"}`}
    >
      <nav
        className={`max-w-7xl mx-auto px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? "py-4" : "py-2"}`}
      >
        {/* Logo */}
        <Link
          to="/"
        ...
        >
          <img
            ...
            alt="LIV Capital"
            className={`w-auto transition-all duration-300 ${scrolled ? "h-20" : "h-12"}`}
          />
        </Link>
